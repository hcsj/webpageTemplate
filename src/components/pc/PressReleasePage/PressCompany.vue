    ...
    <div class="contents">
      ...
      <div class="details" v-if="details">
        <i class="el-icon-close" @click="details=false"></i>
         ...
         <div class="ql-container ql-snow"> <div class="title" v-html="title"></div><section  class="ql-editor" v-html="message"></section></div>
      </div>
    </div>
    <div class="paging">
      <!-- <el-pagination
...
// website/noticeInfo/getNewsPage
...
export default {
  ...
  data() {
    return {
      ...
      tableDataEnd: [],
      //会员咨询跳过来的
      consult: this.$route.params.consultId,
      title:''
    };
  },
  methods: {
    ...
    pass() {
      ...
      if (this.consult) {
        ...
        request({
          ...
          var arr = res.data.body.content;
          for (let i = 0; i < arr.length; i++) {
           if (arr[i].id == this.consult) {
             let consult = arr[i]
              this.open(consult);
           }
          }
          // let consult = res.data.body.content.id;
